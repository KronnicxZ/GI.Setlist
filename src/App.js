...
function App() {
  ...
  const { restoreAlert, setRestoreAlert, handleBackup, handleRestore, confirmRestore } = useBackup(
    API_URL,
    setSuccessAlert,
    setErrorAlert
  );

  const { contextSongs, filteredSongs } = useSongFilters(
    songs,
    selectedSetlist,
    searchTerm,
    genreFilter,
  ...
  const onSaveSong = async (data) => {
    try {
      ...
      console.error(err);
      setErrorAlert({ isOpen: true, message: err.message || 'No se pudo guardar la canción' });
    }
  };
  ...
  const onConfirmDuplicate = async (data) => {
    try {
      ...
      console.error(err);
      setErrorAlert({ isOpen: true, message: err.message || 'No se pudo duplicar la canción' });
    }
  };
  ...
  const onConfirmDeleteSong = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorAlert({ isOpen: true, message: err.message || 'No se pudo eliminar' });
    } finally {
  ...
  const onSaveSetlistHandler = async (data) => {
    try {
      ...
      console.error(err);
      setErrorAlert({ isOpen: true, message: err.message || 'No se pudo guardar el setlist' });
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-main text-white md:gap-4 overflow-hidden">
      <Sidebar
        ...
        setShowSongForm={setShowSongForm}
        setEditingSetlist={setEditingSetlist}
        setShowSetlistForm={setShowSetlistForm}
        activeTab={activeTab}
        ...
        handleDeleteSetlist={onDeleteSetlistHandler}
        setShowAdminPanel={setShowAdminPanel}
      />

      <div
        className={`flex-1 flex flex-col h-full relative overflow-hidden ${selectedSong || showSongForm ? 'hidden md:flex' : 'flex'}`}
      >
        <Header
          selectedSetlist={selectedSetlist}
          setSelectedSetlist={setSelectedSetlist}
          ...
          {activeTab === 'setlists' && (
            <div className="md:hidden space-y-4 pb-10">
              <div
                ...
                className={`p-5 rounded-sub flex justify-between border ${!selectedSetlist ? 'bg-primary/10 border-primary/20' : 'bg-white/5 border-white/5'}`}
              >
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-primary/20 rounded-sub flex items-center justify-center text-primary">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{songs.length} temas</div>
                  </div>
                </div>
              </div>
              {setlists.map((sl) => (
                <div
                  ...
                  className={`p-5 rounded-sub flex justify-between border ${selectedSetlist?.id === sl.id ? 'bg-primary/10 border-primary/20' : 'bg-white/5 border-white/5'}`}
                >
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-white/5 rounded-sub flex items-center justify-center text-gray-400">
                      <svg className="w-6 h-6" viewBox="0 0 24 24">
                        <path
                          fill="currentColor"
                    ...
                    <div>
                      <div className="font-bold text-lg">{sl.name}</div>
                      <div className="text-xs text-gray-500">{sl.songs?.length || 0} temas</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {activeTab === 'admin' && (
            <div className="md:hidden pt-10 text-center px-6">
              ...
              <p className="text-muted mb-10 text-sm">
                {isAdmin ? 'Acceso de Administrador' : 'Explora la biblioteca'}
              </p>
              {isAdmin && (
                <div className="space-y-4">
                  <button
                    onClick={handleBackup}
                    className="w-full p-4 bg-primary/10 text-primary rounded-sub font-bold border border-primary/20"
                  >
                    Respaldar Datos
                  </button>
                  ...
                  <button
                    onClick={logout}
                    className="w-full p-4 bg-red-400/10 text-red-400 rounded-sub font-bold border border-red-400/20"
                  >
                    Cerrar Sesión
                  </button>
                </div>
      ...
      <ModalsContainer
        ...
        setEditingSong={setEditingSong}
        handleSaveSong={onSaveSong}
        songs={songs}
        showSetlistForm={showSetlistForm}
        setShowSetlistForm={setShowSetlistForm}
        ...
        isDeleteSetlistModalOpen={isDeleteSetlistModalOpen}
        setIsDeleteSetlistModalOpen={setIsDeleteSetlistModalOpen}
        setSetlistToDelete={setSetlistToDelete}
        handleConfirmDeleteSetlist={onConfirmDeleteSetlist}
        isDeleteSongModalOpen={isDeleteSongModalOpen}
        setIsDeleteSongModalOpen={setIsDeleteSongModalOpen}
        songToDeleteInfo={songToDeleteInfo}
        setSongToDeleteInfo={setSongToDeleteInfo}
        handleConfirmDeleteSong={onConfirmDeleteSong}
        songToDuplicate={songToDuplicate}
        setSongToDuplicate={setSongToDuplicate}
        ...
        setRestoreAlert={setRestoreAlert}
      ...
      {selectedSetlist && <PrintSetview setlist={selectedSetlist} songs={contextSongs} />}
    </div>
  );
}
