          ...
          {loading ? [...Array(4)].map((_, i) => (
            ...
            songs.map((song) => (
              <div
                ...
                className={`flex items-center space-x-3 p-3 rounded-2xl border transition-transform duration-200 active:scale-[0.97] will-change-transform ${selectedSongs.includes(song.id || song._id) ? 'bg-primary/10 border-primary/40 shadow-lg shadow-primary/5' : 'bg-white/[0.04] border-white/5'}`}
              >
                ...
                <div className="w-12 h-12 rounded-xl overflow-hidden bg-white/5 shrink-0">
                  {(() => {
                    ...
                    return vId ? <img src={`https://img.youtube.com/vi/${vId}/default.jpg`} loading="lazy" className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center text-gray-700 bg-white/5"><svg className="w-6 h-6" viewBox="0 0 24 24"><path fill="currentColor" d="M12,3V13.55C11.41,13.21 10.73,13 10,13C7.79,13 6,14.79 6,17C6,19.21 7.79,21 10,21C12.21,21 14,19.21 14,17V7H18V3H12Z" /></svg></div>
                  })()}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      <span className="text-[9px] font-mono text-gray-400">{song.duration || songDurations[song.id || song._id] || '--:--'}</span>
                    </div>
                  </div>
                </div>
                {isAdmin && (
                  <div className="shrink-0 relative song-menu-container" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setOpenMenuId(openMenuId === (song.id || song._id) ? null : (song.id || song._id))} className="p-2 text-gray-500 active:text-white">
                    ...
                    {openMenuId === (song.id || song._id) && (
                      <div className="absolute right-0 top-full mt-1 w-32 bg-[#0a0a0a] border border-white/10 rounded-xl shadow-2xl z-50 py-1 backdrop-blur-3xl overflow-hidden">
                        <button onClick={() => { onEditSong(song); setOpenMenuId(null); }} className="w-full px-4 py-2.5 text-left text-xs text-gray-300">Editar</button>
                        <button onClick={() => { onDuplicateSong(song); setOpenMenuId(null); }} className="w-full px-4 py-2.5 text-left text-xs text-gray-300">Duplicar</button>
                        <button onClick={() => { onDeleteSong(song.id || song._id); setOpenMenuId(null); }} className="w-full px-4 py-2.5 text-left text-xs text-red-500/70">Eliminar</button>
                      </div>
                    )}
                  </div>
                )}
