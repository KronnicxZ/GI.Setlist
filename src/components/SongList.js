      ...
      >
        {isAdmin && (
          ...
          >
            ...
            >
              {isSelected && (
                <svg className="w-3.5 h-3.5 text-black" viewBox="0 0 24 24">
                  <path
                    fill="currentColor"
        ...
        {isAdmin && (
          <div
            className="shrink-0 relative song-menu-container"
            ...
            {isOpen && (
              <div className="absolute right-0 top-full mt-1 w-32 bg-card border border-white/10 rounded-xl shadow-2xl z-50 py-1 overflow-hidden">
                <button
                  ...
                  className="w-full px-4 py-2.5 text-left text-xs text-gray-300 active:bg-white/5"
                >
                  Editar
                </button>
                <button
                ...
                <button
                  onClick={() => {
                    onDelete(song.id || song._id);
                    onMenuToggle(null);
                  }}
                  className="w-full px-4 py-2.5 text-left text-xs text-red-500/70 active:bg-red-500/10"
                >
                  Eliminar
                </button>
              </div>
...
const EmptyState = ({ onClearFilters }) => (
  <div className="flex flex-col items-center justify-center py-20 px-4 text-center w-full animate-fade-in">
    <div className="w-24 h-24 mb-6 rounded-full bg-gradient-to-tr from-primary/20 to-primary/40 flex items-center justify-center p-0.5 relative group">
      ...
      <div className="w-full h-full bg-[#161616] rounded-full flex items-center justify-center relative z-10 border border-primary/20">
        ...
        >
          <path
            fill="currentColor"
            d="M16 9h-3V3h-2v6H8V3H6v6c0 1.66 1.34 3 3 3v9h2v-9c1.66 0 3-1.34 3-3zM21 3h-2v11h-2V3h-2v11c0 1.66 1.34 3 3 3v4h2v-4c1.66 0 3-1.34 3-3V3z"
          />
        </svg>
      </div>
    </div>
    ...
    <p className="text-gray-400 max-w-sm mb-8 text-sm leading-relaxed">
      Sincroniza tus datos, importa un archivo JSON de respaldo o utiliza el panel de administrador
      para añadir tu primer tema musical.
    </p>
    {onClearFilters && (
      <button
        ...
        <div className="sticky top-0 z-40 bg-[#161616]/95 backdrop-blur-md px-4 md:px-6 py-4 rounded-xl flex items-center justify-between shadow-2xl animate-fade-in mx-1 md:mx-0 mb-6 border border-white/10">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 bg-primary/20 px-3 py-1 rounded-full border border-primary/30">
              <span className="text-primary font-black text-xs md:text-sm uppercase tracking-tighter">
                {selectedSongs.length}
              </span>
              <span className="text-[10px] font-bold text-primary/80 uppercase tracking-widest hidden xs:inline">
                Seleccionados
              </span>
            </div>
            ...
            >
              ...
              <span className="hidden sm:inline">
                {isRemovingFromSetlist ? 'Quitar del Setlist' : 'Eliminar de Biblioteca'}
              </span>
            </button>
          </div>
          ...
          <table className="min-w-full border-separate border-spacing-0">
            <thead>
              <tr className="text-left text-[11px] text-gray-400 uppercase tracking-widest bg-white/[0.04] backdrop-blur-md">
                {isAdmin && (
                  <th className="py-5 px-6 font-bold w-12 border-b border-white/5 text-center">
                    <button
                      onClick={() =>
                        setSelectedSongs(
                          selectedSongs.length === songs.length
                            ? []
                            : songs.map((s) => s.id || s._id)
                        )
                      }
                      className={`w-4 h-4 border-2 rounded transition-all inline-flex items-center justify-center ${selectedSongs.length === songs.length && songs.length > 0 ? 'bg-primary border-primary' : 'border-white/10'}`}
                    >
                      {selectedSongs.length === songs.length && songs.length > 0 && (
                        <svg className="w-2.5 h-2.5 text-black" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"
                          />
                ...
                <th className="py-5 px-6 font-bold border-b border-white/5 hidden lg:table-cell">
                  Género
                </th>
              ...
              {loading ? (
                  ...
                  <tr key={`skeleton-${i}`} className="animate-pulse">
                    {isAdmin && (
                      <td className="py-4 px-6">
                        <div className="w-4 h-4 bg-white/5 rounded mx-auto"></div>
                      </td>
                    )}
                    <td className="py-4 px-6">
                      <div className="flex items-center space-x-4">
                        <div className="w-10 h-10 bg-white/5 rounded-sub"></div>
                        <div className="w-32 h-3 bg-white/5 rounded-full"></div>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <div className="w-24 h-2.5 bg-white/5 rounded-full"></div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="w-6 h-4 bg-white/5 rounded mx-auto"></div>
                    </td>
                    {isAdmin && (
                ...
                songs.map((song) => (
                  ...
                  >
                    {isAdmin && (
                      <td className="py-4 px-6 text-center" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className={`w-4 h-4 border-2 rounded ${selectedSongs.includes(song.id || song._id) ? 'bg-primary border-primary' : 'border-white/10'}`}
                        >
                      ...
                      <div className="flex items-center space-x-3">
                        {(() => {
                          ...
                          return vId ? (
                            ...
                            <div className="w-10 h-10 bg-white/5 rounded flex items-center justify-center text-gray-600">
                              <svg className="w-5 h-5" viewBox="0 0 24 24">
                                <path
                                  fill="currentColor"
                                  d="M12,3V13.55C11.41,13.21 10.73,13 10,13C7.79,13 6,14.79 6,17C6,19.21 7.79,21 10,21C12.21,21 14,19.21 14,17V7H18V3H12Z"
                    ...
                    <td className="py-4 px-6 text-sm text-gray-400 truncate max-w-[150px]">
                      {song.artist || '-'}
                    </td>
                    <td className="py-4 px-6 hidden lg:table-cell text-[10px] uppercase font-bold text-gray-500">
                      {song.genre || '-'}
                    </td>
                    ...
                    {isAdmin && (
                      <td
                        className="py-4 px-6 text-right relative song-menu-container"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <button
                          onClick={() =>
                            setOpenMenuId(
                              openMenuId === (song.id || song._id) ? null : song.id || song._id
                            )
                          }
                        ...
                        {openMenuId === (song.id || song._id) && (
                          <div className="absolute right-6 top-10 w-40 bg-main border border-white/10 rounded-sub shadow-2xl z-50 py-2 text-left backdrop-blur-3xl">
                            <button
                              ...
                              className="w-full px-4 py-2 text-xs hover:bg-white/5"
                            >
                              Editar
                            </button>
                            <button
                            ...
                            <button
                              onClick={() => {
                                onDeleteSong(song.id || song._id);
                                setOpenMenuId(null);
                              }}
                              className="w-full px-4 py-2 text-xs text-red-500/70 hover:bg-red-500/10"
                            >
                              Eliminar
                            </button>
                          </div>
        ...
        <div className="md:hidden flex flex-col space-y-3 w-full pb-8">
          {loading ? (
            [...Array(6)].map((_, i) => (
              <div
                key={`m-sk-${i}`}
                className="bg-white/5 rounded-2xl p-4 animate-pulse h-[74px]"
              ></div>
            ))
          ) : songs.length === 0 ? (
            <EmptyState onClearFilters={onClearFilters} />
