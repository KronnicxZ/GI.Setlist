  ...
  return (
    <div
      ...
      className={`flex items-center space-x-3 p-3 rounded-2xl border transition-all duration-200 active:scale-[0.97] will-change-transform relative ${isOpen ? 'z-[200]' : 'song-card-optimized overflow-hidden'
        } ${isSelected ? 'bg-primary/10 border-primary/40 shadow-lg shadow-primary/5' : 'bg-white/[0.04] border-white/5'}`}
    >
      {isAdmin && (
          ...
          {loading ? [...Array(6)].map((_, i) => (
            ...
            songs.map((song) => (
              <SongCard
                ...
                duration={song.duration || songDurations[song.id || song._id] || '--:--'}
