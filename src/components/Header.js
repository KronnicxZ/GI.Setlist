  ...
  return (
    <>
        ...
        <div className="max-w-[1800px] mx-auto flex justify-between items-center gap-6">
          <div>
            <div className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] mb-1">
              {selectedSetlist ? 'Colección' : 'Música'}
            </div>
            <div className="flex items-baseline space-x-4">
              <h1 className="text-4xl font-extrabold tracking-tight">
                {selectedSetlist ? selectedSetlist.name : 'Biblioteca'}
              </h1>
              <div className="flex items-center space-x-3">
                {selectedSetlist?.date && (
                  <span className="text-sm font-bold text-gray-400 bg-white/5 px-4 py-2 rounded-full border border-white/5">
                    {new Date(selectedSetlist.date).toLocaleDateString('es-ES', {
                      weekday: 'long',
                      day: 'numeric',
                      month: 'long',
                      timeZone: 'UTC',
                ...
                {selectedSetlist && (
                  <span className="text-sm font-bold text-gray-500 px-2 uppercase tracking-widest">
                ...
                {selectedSetlist && (
                  <button
                    ...
                    className="text-gray-500 hover:text-white p-2 hover:bg-white/5 rounded-full transition-colors no-print"
                  >
            ...
            <div className="flex items-center gap-4 flex-1 max-w-2xl justify-end">
              ...
              {isAdmin && (
                <button
                  ...
                  className="bg-primary text-black h-[42px] px-6 rounded-sub font-bold flex items-center justify-center space-x-2 hover:bg-primary-hover shadow-lg active:scale-95 transition-all"
                >
      ...
      <div className="md:hidden sticky top-0 z-50 bg-main/80 backdrop-blur-xl border-b border-white/5 pt-4 pb-2 px-4">
        ...
        {((activeTab === 'library' && !selectedSetlist) || activeTab === 'search') && (
          <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar pb-2">
            {['Todas', 'Alabanza', 'Adoración'].map((genre) => {
              ...
              const count =
                genre === 'Todas'
                  ? contextSongs.length
                  : contextSongs.filter((s) => normalize(s.genre) === normalize(genre)).length;
              return (
                <button
                  key={genre}
