...
const Sidebar = ({
  ...
  return (
    <div
      className={`hidden md:flex flex-col h-screen sticky top-0 z-50 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'w-20' : 'w-72'}`}
    >
      <button
        onClick={(e) => {
          e.stopPropagation();
      ...
      <div
        className={`h-full bg-sidebar border-r border-white/5 overflow-y-auto overflow-x-hidden custom-scrollbar ${isSidebarCollapsed ? 'cursor-pointer' : ''}`}
        onClick={() => isSidebarCollapsed && setIsSidebarCollapsed(false)}
      >
        <div className="flex flex-col min-h-full p-4">
          <div
            className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} mb-10`}
          >
            <div
            ...
            >
              ...
              {!isSidebarCollapsed && (
                <span className="text-xl font-bold tracking-tight text-white truncate animate-fade-in">
                  GI <span className="text-primary">Setlist</span>
                </span>
              )}
            </div>
          </div>
          ...
          <nav className="flex-1 space-y-8">
              ...
              <div>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className={`flex items-center ${isSidebarCollapsed ? 'justify-center py-2' : 'space-x-3 px-4 py-3 rounded-sub hover:bg-white/[0.03]'} text-gray-400 hover:text-primary w-full transition-all group`}
                    title={isSidebarCollapsed ? 'Nueva canción' : ''}
                  >
                    <div
                      className={`${isSidebarCollapsed ? 'w-10 h-10' : 'w-8 h-8'} rounded-lg bg-white/5 flex-shrink-0 flex items-center justify-center group-hover:bg-primary/10 transition-colors`}
                    >
                  ...
                  <button
                    ...
                    className={`flex items-center ${isSidebarCollapsed ? 'justify-center py-2' : 'space-x-3 px-4 py-3 rounded-sub hover:bg-white/[0.03]'} text-gray-400 hover:text-primary w-full transition-all group`}
                    title={isSidebarCollapsed ? 'Nuevo setlist' : ''}
                  >
                    <div
                      className={`${isSidebarCollapsed ? 'w-10 h-10' : 'w-8 h-8'} rounded-lg bg-white/5 flex-shrink-0 flex items-center justify-center group-hover:bg-primary/10 transition-colors`}
                    >
                ...
                >
                  ...
                  >
                    <svg className="w-4 h-4" viewBox="0 0 24 24">
                      <path
                        fill="currentColor"
                        d="M12,13A5,5 0 0,1 7,8H9A3,3 0 0,0 12,11A3,3 0 0,0 15,8H17A5,5 0 0,1 12,13M12,3A3,3 0 0,1 15,6H9A3,3 0 0,1 12,3M19,6H17A5,5 0 0,0 12,1A5,5 0 0,0 7,6H5C3.89,6 3,6.89 3,8V20A2,2 0 0,0 5,22H19A2,2 0 0,0 21,20V8C21,6.89 20.11,6 19,6Z"
                      />
                    </svg>
                  </div>
                  {!isSidebarCollapsed && (
                    <span className="font-medium text-sm">Biblioteca Global</span>
                  )}
                </div>
                {setlists.map((setlist) => (
                  <div
                    key={setlist.id}
                    className={`flex items-center ${isSidebarCollapsed ? 'justify-center py-2' : 'justify-between px-4 py-2 rounded-sub'} group transition-all ${isSidebarCollapsed ? (selectedSetlist?.id === setlist.id && activeTab === 'library' ? 'text-primary' : 'text-gray-400 hover:text-white') : selectedSetlist?.id === setlist.id && activeTab === 'library' ? 'bg-primary/10 text-primary border-l-2 border-primary' : 'text-gray-400 hover:bg-white/[0.03] hover:text-white'} cursor-pointer`}
                    title={isSidebarCollapsed ? setlist.name : ''}
                    onClick={() => {
                      setActiveTab('library');
                      ...
                      <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity space-x-1 setlist-menu-container">
                        {isAdmin && (
                          <button
                            ...
                            className="p-1 hover:text-primary rounded-lg hover:bg-white/5"
                          >
                            <svg className="w-4 h-4" viewBox="0 0 24 24">
                              <path
                                fill="currentColor"
                        ...
                        <div className="relative">
                          <button
                            ...
                            className="p-1 hover:text-white rounded-lg hover:bg-white/5"
                          >
                            <svg className="w-4 h-4" viewBox="0 0 24 24">
                              <path
                                fill="currentColor"
                                d="M12,16A2,2 0 0,1 14,18A2,2 0 0,1 12,20A2,2 0 0,1 10,18A2,2 0 0,1 12,16M12,10A2,2 0 0,1 14,12A2,2 0 0,1 12,14A2,2 0 0,1 10,12M12,4A2,2 0 0,1 14,6A2,2 0 0,1 12,8A2,2 0 0,1 10,6A2,2 0 0,1 12,4Z"
                              />
                            </svg>
                          </button>
                          {openMenuId === setlist.id && (
                            <div className="absolute right-0 top-full mt-1 w-32 bg-card border border-white/10 rounded-sub shadow-2xl py-1 z-50 overflow-hidden animate-fade-in glass">
                              {isAdmin && (
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="block w-full text-left px-4 py-2 text-xs text-red-400 hover:bg-red-400/10 font-medium"
                                >
                                  Eliminar
                                </button>
                              )}
            ...
            <div>
              ...
              <div className="space-y-1">
                ...
                >
                  <div
                    className={`${isSidebarCollapsed ? 'w-10 h-10' : 'w-8 h-8'} rounded-lg bg-white/5 flex-shrink-0 flex items-center justify-center ${activeTab === 'tools' ? 'text-primary border-primary' : ''}`}
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24">
                      <path
                        fill="currentColor"
                        d="M12.5,7V12.25L17,14.92L16.25,16.15L11,13V7H12.5M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M12,4A8,8 0 0,1 20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4Z"
                      />
                    </svg>
                  </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/5">
            {isAdmin && (
              <button
                ...
                className={`flex items-center ${isSidebarCollapsed ? 'justify-center py-2' : 'space-x-3 px-4 py-3 rounded-sub hover:bg-white/[0.03]'} text-gray-400 hover:text-primary w-full transition-all group`}
                title={isSidebarCollapsed ? 'Panel de Admin' : ''}
              >
                <div
                  className={`${isSidebarCollapsed ? 'w-10 h-10' : 'w-8 h-8'} rounded-lg bg-white/5 flex-shrink-0 flex items-center justify-center group-hover:bg-primary/10 transition-colors`}
                >
