...
const TUNING_REFS = [
  ...
  { note: 'A2', freq: 110.0, string: '5ª' },
  { note: 'D3', freq: 146.83, string: '4ª' },
  { note: 'G3', freq: 196.0, string: '3ª' },
    ...
    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-[#121212] rounded-3xl border border-white/5 shadow-[0_0_40px_rgba(0,0,0,0.5)] w-full max-w-lg mx-auto relative overflow-hidden">
          ...
          <p className="text-xs text-gray-500 font-medium">
            {isActive
              ? isTuned
                ? '✓ Afinado'
                : 'Escuchando...'
              : 'Usa el micrófono de tu dispositivo'}
          </p>
        </div>
      </div>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center w-full">
          {isActive ? (
            <div
              className={`relative flex flex-col items-center justify-center w-52 h-52 md:w-64 md:h-64 rounded-full border-4 transition-all duration-300 shadow-xl mb-6 ${isTuned ? 'border-green-500 bg-green-500/10 shadow-[0_0_30px_rgba(34,197,94,0.4)]' : cents > 0 ? 'border-orange-500/50 bg-orange-500/5' : cents < 0 ? 'border-blue-500/50 bg-blue-500/5' : 'border-white/10 bg-white/5'}`}
            >
              <div
                className={`text-6xl md:text-7xl font-black tracking-tighter ${isTuned ? 'text-green-400' : 'text-white'}`}
              >
                {noteName}
              </div>
              ...
              >
                <div
                  className={`w-2.5 h-2.5 rounded-full -ml-1 -mt-1 ${isTuned ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.8)]' : 'bg-white'}`}
                ></div>
              </div>
            </div>
          ) : (
            <div className="w-52 h-52 md:w-64 md:h-64 rounded-full border-4 border-white/5 bg-white/[0.02] flex flex-col items-center justify-center mb-6">
              <svg className="w-12 h-12 text-gray-600 mb-2" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
          ...
          {isActive && (
            <div className="w-full mb-6">
              ...
              <div className="text-center mt-2">
                ...
                <span className="text-[10px] font-medium text-gray-500 tracking-wider ml-1.5">
                  CENTS
                </span>
              </div>
            </div>
          ...
          {isActive && (
            <div className="w-full mb-6">
              <h4 className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-2 ml-1">
                Afinación Estándar
              </h4>
              <div className="grid grid-cols-6 gap-1">
                {TUNING_REFS.map((ref, i) => {
                  const isClosest =
                    note !== null && noteStrings[note % 12] === ref.note.replace(/[0-9]/g, '');
                  return (
                    <div
                      key={i}
                      className={`flex flex-col items-center p-1.5 rounded-lg border transition-all ${isClosest && isTuned ? 'bg-green-500/15 border-green-500/30' : isClosest ? 'bg-white/10 border-white/20' : 'bg-white/[0.03] border-white/5'}`}
                    >
                      <span className="text-[9px] text-gray-500 font-bold">{ref.string}</span>
                      <span
                        className={`text-xs font-bold ${isClosest ? (isTuned ? 'text-green-400' : 'text-white') : 'text-gray-500'}`}
                      >
                        {ref.note}
                      </span>
                    </div>
