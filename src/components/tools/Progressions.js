...
const progressions = [
  {
    ...
    baseChords: ['C', 'G', 'Am', 'F'],
    description:
      "La progresión más usada en la música popular y de adoración. Usada en canciones como 'Let It Be' y 'Oceans'.",
  ...
  },
  {
    ...
    baseChords: ['Am', 'F', 'C', 'G'],
    description:
      "Muy común en coros intensos y bridges modernos. Usada en 'Reckless Love' y muchas baladas.",
  ...
  },
  {
    ...
    baseChords: ['C', 'F', 'G', 'F'],
    description:
      'Para ritmos rápidos, júbilo y celebración. Perfecta para alabanza rítmica y danza.',
    mood: 'Alegre / Movido',
  ...
  },
  {
    ...
    baseChords: ['F', 'C', 'G', 'Am'],
    description:
      'Inicia en la subdominante, creando tensión y anhelo. Muy usada en Hillsong y Bethel.',
    mood: 'Clamor / Tensión',
  ...
  },
  {
    ...
    baseChords: ['Dm', 'G', 'C'],
    description:
      'La base de modulaciones y progresiones ricas armónicamente. Esencial para gospel.',
    mood: 'Sofisticado',
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <h3
                  className={`font-bold text-base md:text-lg ${isActive ? 'text-white' : 'text-gray-300'}`}
                >
                  {prog.name}
                </h3>
                <span
                  className={`text-[9px] uppercase font-black tracking-widest px-2.5 py-1 rounded-full border ${isActive ? 'text-white bg-white/10 border-white/20' : 'text-gray-500 bg-black/50 border-white/10'}`}
                >
                  {prog.mood}
                </span>
              </div>
              ...
              <div className="flex gap-2 mb-2">
                {prog.roman.map((numeral, i) => (
                  <div
                    key={i}
                    className={`flex-1 flex items-center justify-center py-1.5 md:py-2 rounded-lg font-bold text-xs md:text-sm ${isActive ? 'bg-black/30 text-white/80 border border-white/10' : 'bg-black/20 text-gray-500 border border-white/5'}`}
                  >
                    {numeral}
                  </div>
                ))}
              ...
              <div className="flex gap-2 mb-2">
                {transposed.map((chord, i) => (
                  <div
                    key={i}
                    className={`flex-1 flex items-center justify-center py-2 md:py-2.5 rounded-lg font-black text-sm md:text-base shadow-inner ${isActive ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-white/5 text-gray-300 border border-white/5'}`}
                  >
                    {chord}
                  </div>
                ))}
