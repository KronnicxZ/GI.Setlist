...
const ChatAI = ({ isPopup = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: 'Error de conexión con el asistente. Verifica tu conexión a internet.',
        },
      ]);
    }
    setLoading(false);
  ...
  return (
    <div
      className={`w-full mx-auto flex flex-col bg-[#121212] overflow-hidden relative ${isPopup ? 'h-full rounded-2xl md:rounded-3xl border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]' : 'max-w-4xl h-[calc(100vh-220px)] md:h-[70vh] border border-white/10 rounded-2xl md:rounded-3xl shadow-2xl animate-fade-in'}`}
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 custom-scrollbar">
        {messages.map((msg, idx) => (
          ...
          >
            <div
              className={`max-w-[85%] md:max-w-[75%] rounded-2xl px-4 py-3 ${msg.role === 'user' ? 'bg-primary text-black rounded-br-sm shadow-lg' : 'bg-white/5 border border-white/10 text-gray-200 rounded-bl-sm'}`}
            >
              <div
      ...
      <form
        ...
        className="p-3 md:p-4 border-t border-white/5 bg-white/[0.02] flex-shrink-0"
      >
