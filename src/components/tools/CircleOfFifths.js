  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-[#121212] rounded-3xl border border-white/5 shadow-2xl animate-fade-in w-full max-w-lg mx-auto relative overflow-hidden">
      ...
      <div className="w-full relative z-10 mt-2 mb-8 flex justify-center">
        <div
          className="relative w-64 h-64 md:w-72 md:h-72 rounded-full border-2 border-white/10"
        ...
        >
          {keysInfo.map((info, index) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[7px] md:text-[8px] mt-0.5 ${isSelected ? 'text-black/70' : 'text-gray-500'}`}
                >
                  {info.minor.split('/')[0]}
                </span>
              </div>
          ...
          <div className="absolute inset-0 m-auto w-28 h-28 md:w-32 md:h-32 rounded-full bg-white/5 border border-white/10 flex flex-col items-center justify-center shadow-inner text-center p-2">
            ...
            <span className="text-[9px] bg-white/10 px-2 py-0.5 mt-1.5 rounded-full font-bold text-gray-400 border border-white/5">
              {selectedKey.sig}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 w-full mb-6">
        <div className="bg-white/5 p-3 md:p-4 rounded-xl border border-white/10 flex flex-col items-center text-center">
          ...
          <span className="text-lg md:text-xl font-black text-white">
            {keysInfo[(selectedIndex + 11) % 12].major.split('/')[0]}
          </span>
        </div>
        <div className="bg-white/5 p-3 md:p-4 rounded-xl border border-white/10 flex flex-col items-center text-center">
          <span className="text-[9px] text-gray-500 uppercase tracking-widest font-bold mb-1">
          ...
          <span className="text-lg md:text-xl font-black text-primary drop-shadow-[0_0_5px_rgba(234,179,8,0.5)]">
            {keysInfo[(selectedIndex + 1) % 12].major.split('/')[0]}
          </span>
        </div>
      </div>
      ...
      <div className="w-full">
        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 ml-1">
          Acordes en {selectedKey.major.split('/')[0]} Mayor
        </h3>
        <div className="grid grid-cols-7 gap-1.5">
          {chordsInKey.map((chord, i) => (
            <div
              key={i}
              className={`flex flex-col items-center p-1.5 md:p-2 rounded-lg border transition-all ${i === 0 ? 'bg-primary/15 border-primary/30' : i === 3 || i === 4 ? 'bg-white/8 border-white/15' : 'bg-white/5 border-white/5'}`}
            >
              <span
                className={`text-[9px] md:text-[10px] font-bold mb-0.5 ${i === 0 ? 'text-primary' : 'text-gray-500'}`}
              >
                {chord.numeral}
              </span>
              <span className="text-[10px] md:text-xs font-bold text-white leading-tight text-center">
                {chord.note}
              </span>
            </div>
          ))}
