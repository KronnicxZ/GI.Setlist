  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-[#121212] rounded-3xl border border-white/5 shadow-2xl animate-fade-in w-full max-w-lg mx-auto relative overflow-hidden">
      ...
      <div className="text-center w-full relative z-10">
        ...
        <div className="mb-2">
          <div
            className={`text-[80px] md:text-[100px] font-black tabular-nums text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-500 leading-none transition-all duration-300 ${isAnimating ? 'scale-105' : 'scale-100'}`}
          >
            {bpm > 0 ? bpm : '--'}
          </div>
        ...
        {tapCount > 0 && (
          <div className="flex items-center justify-center space-x-1.5 mb-6 animate-fade-in">
            {[...Array(Math.min(tapCount, 8))].map((_, i) => (
              <div
                key={i}
                className="w-2 h-2 rounded-full bg-green-500/60 animate-fade-in"
        ...
        >
          ...
          <span className="text-[10px] font-bold opacity-70 tracking-wide uppercase">
            Espacio / Click
          </span>
        </button>
